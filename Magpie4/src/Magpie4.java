...
public class Magpie4 {
	...
	public String getResponse(String statement) {
		...
		if (findKeyword(statement, "no", 0) >= 0) {
			...
			response=("That is a good teacher.");
		}else if (findKeyword(statement,"fam")>=0){
			response=("Was good homie!!!");
		}else if (findKeyword(statement,"problems")>=0){
			response=("That sounds like a you problem. :D");
		}else if (statement.trim().length()==0){
			response="Type something plz.";
		}else if(findKeyword(statement,"basketball")>=0){
			response="Steph Curry sucks.";
	...
	private String getRandomResponse() {
		...
		int whichResponse = (int) (n * NUMBER_OF_RESPONSES);
		String response = "";
